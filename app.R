...
ui <- fluidPage(
  ...
  fluidRow(
    ...
    column(6,
           ...
           textInput("new_admin_token", "Set Admin Token for ID:"),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$create_conf, {
    ...
    token <- trimws(input$new_admin_token)
    if (id == "" || id %in% names(global_data$conferences) || token == "") {
    ...
    updateTextInput(session, "new_admin_token", value = "")
  ...
  observeEvent(input$add_question, {
    req(input$new_question_text, input$conf_id_select)
    conf <- input$conf_id_select
  ...
  observeEvent(input$delete_question, {
    req(input$delete_question_id, input$conf_id_select)
  ...
  observeEvent(input$restart_question, {
    req(input$restart_question_id, input$conf_id_select)
    conf <- input$conf_id_select
  ...
  observeEvent(input$submit_answer, {
    ...
    global_data$conferences[[conf]][[qid]]$responses <- rbind(responses, new_entry)
